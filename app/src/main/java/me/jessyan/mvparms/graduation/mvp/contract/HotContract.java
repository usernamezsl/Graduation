...
public interface HotContract {
    ...
    interface View extends BaseView {
        ...
        void showBanner(GankItemBean gankItemBean);

        void showContent(List<HotContentBean.DataBean> dataBeen);

        void showContentFromDb(List<HomeData> homeDatas);
    }

    //Model层定义接口,外部只需关心model返回的数据,无需关心内部细节,及是否使用缓存
    interface Model extends IModel {

        Observable<GankItemBean> getMeizhiData(int num);
